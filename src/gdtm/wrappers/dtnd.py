...
class dTNDMallet(utils.SaveLoad, basemodel.BaseTopicModel):
    ...
    def load(cls, *args, **kwargs):
        ...
        """
        model = super(dTNDMallet, cls).load(*args, **kwargs)
...
class deTNDMallet(utils.SaveLoad, basemodel.BaseTopicModel):
    ...
    def __init__(self, mallet_path, corpus=None, num_topics=100, alpha=50, beta=0.01, id2word=None, workers=4, prefix=None,
                 optimize_interval=0, iterations=1000, topic_threshold=0.0, random_seed=0, noise_words_max=100, skew=0,
                 tau=200, embeddings_path=None, closest_x_words=3, save_embedding_comp=True,
                 alpha_array_infile=None, tw_dist_file=None, noise_dist_file=None):
        """
    ...
    def get_closest_embedded_words(self):
        if os.path.exists('data/cw_{}_{}.txt'.format(self.embeddings_path.replace('.bin', ''), self.closest_x_words)):
            return
        vec = load_facebook_vectors(self.embeddings_path)
        vec.fill_norms()
        hasht = dict()
    ...
    def load(cls, *args, **kwargs):
        ...
        older LdaMallet versions which did not use random_seed parameter.
        """
        model = super(NftMallet, cls).load(*args, **kwargs)
        if not hasattr(model, 'random_seed'):
            model.random_seed = 0
